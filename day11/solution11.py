...
def test_password(password: str) -> bool:
    ...
    straights = [''.join(c) for c in zip(VALID_CHARS[:-2], VALID_CHARS[1:-1], VALID_CHARS[2:])]
    ...
    # RULE3: must contain a straight of 3 or more consecutive valid characters
...
def next_password(password: str) -> str:
    ...
    next_letter = {char1: char2 for char1, char2 in zip(VALID_CHARS, VALID_CHARS[1:]+'a')}
    ...
    for i in range(-1, -8, -1):
        if password[i] == 'a':
            password = password[:i - 1] + next_letter[password[i - 1]] + password[i:]
        else:
            break
